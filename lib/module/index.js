...
const electronAPI = Platform.OS === "web" ? require("./electron/renderer").electronAPI : undefined;
...
var RNFS = {
  mkdir(filepath, options = {}) {
    return RNFSManager.mkdir(normalizeFilePath(filepath), options).then(() => undefined);
  },
  moveFile(filepath, destPath, options = {}) {
    return RNFSManager.moveFile(normalizeFilePath(filepath), normalizeFilePath(destPath), options).then(() => undefined);
  },
  copyFile(filepath, destPath, options = {}) {
    return RNFSManager.copyFile(normalizeFilePath(filepath), normalizeFilePath(destPath), options).then(() => undefined);
  },
  ...
  copyFileAssets(filepath, destPath) {
    ...
    return RNFSManager.copyFileAssets(normalizeFilePath(filepath), normalizeFilePath(destPath)).then(() => undefined);
  },
  // Android only
  copyFileRes(filename, destPath) {
    ...
    return RNFSManager.copyFileRes(filename, normalizeFilePath(destPath)).then(() => undefined);
  },
  // iOS only
  ...
  writeFile(filepath, contents, encodingOrOptions) {
    ...
    return RNFSManager.writeFile(normalizeFilePath(filepath), b64, options).then(() => undefined);
  },
  ...
  write(filepath, contents, position, encodingOrOptions) {
    ...
    return RNFSManager.write(normalizeFilePath(filepath), b64, position).then(() => undefined);
  },
  ...
  touch(filepath, mtime, ctime) {
    ...
    return RNFSManager.touch(normalizeFilePath(filepath), mtime && mtime.getTime() || 0, ctimeTime);
  },
  scanFile(path) {
