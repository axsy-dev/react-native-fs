...
export class Uploader {
  ...
  async performUpload(params, result) {
    const crlf = "\r\n";
    const twoHyphens = "--";
    const boundary = "*****";
    ...
    for (const file of params.files) {
      ...
      const fileHeaderType = twoHyphens + boundary + crlf + 'Content-Disposition: form-data; name="' + name + '"; filename="' + filename + '"' + crlf + "Content-Type: " + filetype + crlf;
    ...
    const requestLength = totalFileLength + stringData.length + params.files.length * crlf.length;
              ...
              await new Promise((resolve, reject) => {
                fileStream.on("data", chunk => {
                  ...
                  if (params.onUploadProgress) {
                    params.onUploadProgress(totalFileLength - tail.length, byteSentTotal);
                  }
                });
                fileStream.on("end", () => {
